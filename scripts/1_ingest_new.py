...
DOCUMENT_STATUS_FILE = config.DOC_STATUS_FILE # 用于记录文档状态的文件

#embedding模型选择
#---本地模型
LOCAL_BGE_M3_MODEL_PATH = config.LOCAL_BGE_PATH
RAG_EMBEDDING_MODEL_NAME = str(LOCAL_BGE_M3_MODEL_PATH)
#---云端模型----写在create_database函数中了，不再赘述

# =======================================================
# 自定义元数据设置
# =======================================================
DEFAULT_PROJECT_NAME = config.DEFAULT_PROJECT_NAME
MARKDOWN_DOC_VERSION = config.MARKDOWN_DOC_VERSION

EXTERNAL_METADATA_EXCEL = config.EXTERNAL_METADATA_EXCEL  # 外部标注Excel文件名
EXTERNAL_METADATA_CHUNK_ID_COL = config.EXTERNAL_METADATA_CHUNK_ID_COL
EXTERNAL_METADATA_CATEGORY_COL = config.EXTERNAL_METADATA_CATEGORY_COL

# =======================================================
# --- 配置模式选择 ---
# 设置为 True 进行增量更新 (只处理修改过的文件和新文件)
# 设置为 False 进行全量重建 (清空数据库，从头开始)
INCREMENTAL_UPDATE_DEFAULT = config.INCREMENTAL_UPDATE_DEFAULT
# =======================================================
...
def create_database(incremental_update: bool = INCREMENTAL_UPDATE_DEFAULT):  # 使用顶部定义的默认值
    ...
    print(f"--- [{datetime.datetime.now()}] 步骤 3: 初始化 Embedding 模型 ---")
    if config.EMBEDDING_MODE == "LOCAL":
        ...
        print(f"☁️ [模式：CLOUD] 正在连接硅基流动 API: {config.EMBEDDING_MODEL_CLOUD}")
        ...
        print(f"正在使用硅基流动模型 '{config.EMBEDDING_MODEL_CLOUD}' 生成 embeddings...")
